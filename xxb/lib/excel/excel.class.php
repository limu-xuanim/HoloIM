...
class excel extends model
{
    ...
    public function setStyle($excelSheet, $endRow)
    {
        ...
        foreach($this->excelKey as $key => $letter)
        {
            ...
            if(is_array($this->config->excel->titleFields) && in_array($key, $this->config->excel->titleFields)) $excelSheet->getColumnDimension($letter)->setWidth($titleWidth);
            ...
            if(is_array($this->config->excel->centerFields) && in_array($key, $this->config->excel->centerFields))
            {
                $centerStyle = array(
                    'font'    => array(
                        'size' => 9
            ...
            if(strpos($key, 'Date') !== false or (is_array($this->config->excel->dateFields) && in_array($key, $this->config->excel->dateFields)))
            {
                $dateFormat = array(
                    'font'    => array(
                        'size' => 9
