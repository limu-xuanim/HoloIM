...
class baseHTML
{
     ...
     * @param int    $width    宽度，默认根据图标类型自动设置
     * @param int    $height   高度，默认根据图标类型自动设置
     * @param string $class    CSS 类名
     * @param array  $attrs    额外的 SVG 属性
     * @static
     * @access public
     * @return string SVG 标签
     */
    public static function svgIcon($name, $width = null, $height = null, $class = '', $attrs = array())
    {
        helper::import(dirname(__FILE__) . '/../front/svg-icons.php');
        
        $icon = svgIcons::getIcon($name);
        if($icon === false) {
            return '';
        }
        $iconPath = $icon['path'];
        ...
        if($width === null || $height === null) {
            $iconSize = $icon['size'];
            if($iconSize !== false) {
                $width = $width === null ? $iconSize[0] : $width;
                $height = $height === null ? $iconSize[1] : $height;
            } else {
        ...
        return "<svg width=\"{$width}\" height=\"{$height}\" viewBox=\"{$viewBox}\" fill=\"none\" xmlns=\"http://www.w3.org/2000/svg\"{$classAttr}{$extraAttrs}>{$iconPath}</svg>";
    }
