...
class baseHelper
{
    ...
    static public function createLink($moduleName, $methodName = 'index', $vars = '', $viewType = '', $onlyBody = false)
    {
        ...
        if($config->requestType == 'GET')
        {
            ...
            foreach($vars as $key => $value) $link .= "&$key=" . urlencode($value);
    ...
    public static function getRemoteIp()
    {
        ...
        if(!empty($_SERVER["HTTP_X_FORWARDED_FOR"])) $ip = strpos($_SERVER["HTTP_X_FORWARDED_FOR"], ',') ? trim(current(explode(',', $_SERVER["HTTP_X_FORWARDED_FOR"]))) : $_SERVER["HTTP_X_FORWARDED_FOR"];
