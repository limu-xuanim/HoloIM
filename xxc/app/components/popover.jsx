 ...
 * @param {String|JSX.Element|Function} content 提示面板内容
 * @param {Object} props DisplayLayer 组件属性
 * @param {?Function} callback 操作完成时的回调函数
 * @returns {DisplayLayer} DisplayLayer 组件
 * @function
 */
export const showPopover = (position, content, props = {}, callback = null) => {
    ...
    const {target = document.activeElement} = position;
    ...
    className = classes('popover layer -rounded', className, `placement-${placement}`);
    ...
    props = {
        backdropClassName: 'clean', animation: `scale-from-${arrowPlacementMap[placement]}`, ...props, className, style, content, footer, plugName: 'popover'
    };
    delete props.width;
    delete props.height;
    delete props.arrow;
