...
/**
 * 格式化快捷键组合字符串
 * @param decoration 快捷键组合字符串
 * @returns 快捷键组合字符串
 * @function
 */
export const formatKeyDecoration = (decoration: string): string => {
    if (decoration) {
        if (isWindowsOS) {
 ...
 * @returns 快捷键组合字符串
 */
export const getKeyDecoration = (event: KeyboardEvent|React.KeyboardEvent<HTMLInputElement>): string => {
    const {keyCode} = event;
    ...
    type key = keyof typeof specialKeys;
    if (keyCode && !modifyKeys.has(keyCode)) {
        if (specialKeys[keyCode as key]) {
            shortcut.push(specialKeys[keyCode as key]);
        } else {
 ...
 * 快捷键组合字符串中是否仅仅包含修饰键
 * @param decoration 快捷键组合字符串
 * @returns 如果为 `true` 表示仅包含修饰就
 */
export const isOnlyModifyKeys = (decoration: string): boolean => {
    if (!decoration) {
        return false;
    }
