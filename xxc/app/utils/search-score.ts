 ...
 * @private
 */
const calcConditionScore = (condition: Condition, key:string, obj:{[key:string|number|symbol]:any}):number => {
    let score = 0;
    ...
    if (condition.array && Array.isArray(source)) {
 ...
 * ];
 *
 * @param conditions 条件描述对象
 * @param obj 要检查匹配的对象
 * @param keys 搜索关键词组
 * @function
 * @returns 返回一个分值作为比较，分值越大表示越匹配
 */
export const matchScore = (conditions:Condition[], obj: {[key:string|number|symbol]:any}, keys:string[]):number => {
