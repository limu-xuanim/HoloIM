 ...
 * @returns 日期时间对象
 */
export const createDate = (date: DateLike = new Date()): Date => {
 ...
 */
export const isToday = (date: DateLike, now: DateLike = new Date()): boolean => isSameDay(createDate(now), date);
 ...
 */
export const isYesterday = (date: DateLike, now: DateLike = new Date()): boolean => isSameDay(createDate(now).getTime() - TIME_DAY, date);
 ...
 */
export const isTomorrow = (date: DateLike, now: DateLike = new Date()): boolean => isSameDay(createDate(now).getTime() + TIME_DAY, date);
 ...
 */
export const isDBY = (date: DateLike, now: DateLike = new Date()): boolean => isSameDay(createDate(now).getTime() - 2 * TIME_DAY, date);
...
export const formatDate = (date: DateLike, format = 'yyyy-MM-dd hh:mm'): string => {
    ...
    Object.keys(dateInfo).forEach((k: keyof typeof dateInfo) => {
 ...
 * @returns 时间范围描述文本
 */
export const formatDateSpan = (date1: DateLike, date2: DateLike, options: {full?: string; month?: string; day?: string; str?: string;}): string => {
    const format = {
        full: 'yyyy-M-d', month: 'M-d', day: 'd', str: '{0} ~ {1}', ...options
    };
    ...
    return format.str.replace('{0}', date1Str).replace('{1}', date2Str);
};
...
export function formatTime(time: number) {
    return time >= 1000
        ? formatDate(new Date(time), time > 3600000 ? 'h小时m分s秒' : time > 60000 ? 'm分s秒' : 's秒')
        : `${(time / 1000).toFixed(2)}秒`;
}
 ...
 * @param now 当前日期对象
 * @returns 时间描述文本
 * @todo 考虑使用 `Intl.RelativeTimeFormat` 重构
 */
export const describeDateFromNow = (Lang: LangHelper, date: DateLike, now?: DateLike): string => {
    date = createDate(date);
    now = now ? createDate(now) : new Date();
    const minutes = (now.getTime() - date.getTime()) / 60000;
    if (minutes < -20000000) {
        // 客户端所存储的时间有可能是远大于当前时间的（错误地乘了一千），尝试处理这种情况
        console.warn('Cannot describe such date: ', date);
        return describeDateFromNow(Lang, date.getTime() / 1000, now);
    }
    if (minutes < 2) {
        return Lang.string('time.justNow');
    }
    if (minutes < 60) {
        return Lang.format('time.minuteAgo', Math.floor(minutes));
    }
    if (isToday(date, now)) {
        return formatDate(date, 'hh:mm');
    }
    if (isYesterday(date, now)) {
        return Lang.string('time.yesterday');
    }
    if (isDBY(date, now)) {
        return Lang.string('time.dby') || formatDate(date, Lang.string('time.format.month'));
    }
    if (isSameYear(date, now) || (now.getTime() - date.getTime()) <= 31 * TIME_DAY) {
        return formatDate(date, Lang.string('time.format.month'));
    }
    return formatDate(date, Lang.string('time.format.yearMonth'));
};
 ...
 * @returns 时间文本形式
 */
export function getShortTextOfDate(Lang: LangHelper, date: DateLike, now?: DateLike): string {
    date = createDate(date);
    now = now ? createDate(now) : new Date();
    ...
    if (isYesterday(date, now)) {
        return formatDate(date, `${Lang.string('time.yesterday')} hh:mm`);
    }
