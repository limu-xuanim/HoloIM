...
format files and directories using the format command and the --write option:
```bash
npx @biomejs/biome format <files> --write
```

lint files and directories using the lint command:
```bash
...
# Git Hooks
安装 lefthook 以管理 git hooks。
每次提交代码时，lefthook 会自动运行 format 和 lint 命令，确保代码风格一致。
[安装参考](https://github.com/evilmartians/lefthook/blob/master/docs/install.md)
